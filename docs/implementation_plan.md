...
- [x] **V11.02** Loader đọc CrossLLM/Direct/T0 từ archive nhưng không thay đổi raw response, request hash, slot order hoặc retry receipts. `load_archives()` nhận arm label độc lập với model/backbone, giữ nguyên `raw_row` cùng tuple slot/receipt, và biểu diễn T0 providerless bằng slot-aligned empty raw responses.
...
- [x] **V11.05** Reject archive malformed, thiếu 8 slots, thiếu provider receipt đối với X/P, sai token budget, sai public pack hoặc campaign trùng trong cùng arm. T0 chỉ được providerless khi `track=T0`, đồng thời có đúng 8 slots và không có budget/provider receipt; loader kiểm tra one-row JSONL, campaign identity/plan, canonical indices, HTTP/transport receipt, budget status, public pack và duplicate campaign ID.
...
- [x] **V11.14** Resolve toàn bộ `SymbolRef` sang runtime bindings; kiểm tra type, state, domain và observation availability. Mỗi mismatch trở thành diagnostic riêng. Runtime adapter cũng tách `executable_action_ids` khỏi `unsupported_action_ids`, để một action chưa bind không thể lọt vào witness allowlist nhưng không chặn các action khác đã thực thi được.
...
- [ ] **V11.16** Nối predicate với bounded search backend phù hợp. Ghi rõ backend là paired fixture, source-backed symbolic hay concrete bounded exploration. `FixtureVerificationExecutors.symbolic_search` đã nối XLIR predicate với bounded paired fixture explorer; `SourceBackedVerificationExecutors` nay có JSON subprocess boundary và truyền clean case workspace/identity tới tool, nhưng tool symbolic source-backed cụ thể và execution evidence vẫn pending.
- [x] **V11.17** Propagate bounds, deadline, cancellation và status `SAT/BOUNDED_UNSAT/UNKNOWN/TIMEOUT/UNSUPPORTED/CRASH`. `SourceBackedVerificationExecutors` truyền nguyên request/bounds, dùng timeout/cancellation không shell và map đầy đủ status; test timeout và malformed/incomplete result giữ missingness.
- [ ] **V11.18** Với `SAT` complete, project witness gồm initial-state hash, actions, callers, calldata, domains, observations và trace hash. Shared source protocol nay kiểm tra các identity/hash và trường witness bắt buộc, lưu witness canonical cùng `witness_hash` trong outcome evidence; request đã bind `lineage_id/case_id/instance_id` và source identity của workspace, nhưng witness do backend source-backed thật sinh cùng evidence deployment/action trên evaluation corpus vẫn pending.
- [ ] **V11.19** Chạy native/concrete witness check từ clean initial state; không dùng state còn lại từ symbolic search. Source executor nay materialize source/test overlay độc lập cho search, witness và replay; fixture executor reset/replay từ snapshot ban đầu. Lệnh witness/replay thật trên evaluation corpus vẫn pending.
- [x] **V11.20** Gọi independent EVM replay adapter với runtime spec pinned; xác minh trace hash, deployment identity và property observation. `SourceBackedVerificationExecutors` gọi `IndependentEVMReplay`, kiểm tra trace/property/security-relevance và source witness đã bind runtime/artifact/deployment hashes; test mismatch fail-closed. Receipt EVM thật trên corpus vẫn pending.
...
- [x] **V11.22** Cache theo `(case runtime hash, XLIR canonical hash, adapter revision, bounds, replay spec hash, executor bundle hash)`; `FileVerificationCache` ghi atomically, không cho overwrite cùng key bằng outcome khác, và pipeline đánh dấu `cache_hit` để không chạy lại stage terminal. Source-backed bundle hash bao phủ search, witness, replay, tool revision, immutable container và workdir nên đổi backend không thể tái sử dụng `unsupported`/partial result cũ.
- [x] **V11.23** Test wrong property, missing binding, altered witness, wrong initial state, wrong caller, invalid calldata, control case, timeout, unsupported opcode và replay trace mismatch. Coverage is distributed across `tests/unit/test_fixture_verification.py`, `tests/unit/test_runtime_binding.py`, `tests/unit/test_witness_replay.py`, `tests/unit/test_replay_negative_controls.py` and `tests/unit/test_source_verification.py`; all relevant suites pass.

Implementation increment 2026-09-12: `source_workspace.py` now verifies the public source/test receipts, resolves repository-prefix mappings from `source_manifest.json`, and creates a disposable clean harness with the case source and paired test overlaid. `scripts/validate_source_case_workspaces.py` validates all 240 public source/test bundles without compilation; the current run is 240/240 pass. `SourceBackedVerificationExecutors` uses a new workspace for each downstream stage, passes source identity to the external command, and supports the explicit `allow_dynamic_harness_bindings` configuration for addresses created by an isolated harness `setUp`. `IndependentEVMReplay` accepts the `{workspace}` argument placeholder. This is infrastructure evidence only; it does not promote the existing fixed Solidity probes or fixture commands to candidate-specific evaluation evidence.
...
- [x] **V11.35** Cho T0 dùng đúng runtime adapter, search bounds, witness checker, replay adapter và analysis exporter của X/P. `verify_method_run()`/`verify_method_campaign()` chuyển T0 `MethodRun` vào cùng pipeline, giữ cùng bounds/cache/metrics contract; runtime-event projection và `run_t0_campaign.py` cũng hỗ trợ providerless T0 mà không tạo receipt giả. Test fixture đã chạy đủ grounding → symbolic search → witness check → independent replay và tạo Recall@1. Source-backed EVM executor vẫn là phạm vi V11.16–V11.20, không được suy diễn từ fixture test.
