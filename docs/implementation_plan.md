...
- [ ] **V11.18** Với `SAT` complete, project witness gồm initial-state hash, actions, callers, calldata, domains, observations và trace hash. Shared source protocol nay kiểm tra các identity/hash và trường witness bắt buộc, lưu witness canonical cùng `witness_hash` trong outcome evidence; witness do backend source-backed thật sinh và evidence deployment/action trên evaluation corpus vẫn pending.
- [ ] **V11.19** Chạy native/concrete witness check từ clean initial state; không dùng state còn lại từ symbolic search. Fixture executor reset/replay từ snapshot ban đầu; full case harness integration vẫn pending.
