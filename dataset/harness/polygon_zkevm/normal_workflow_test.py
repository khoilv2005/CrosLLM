"""Harness test executing a pinned-container EVM workflow for polygon_zkevm (Polygon zkEVM / Agglayer)."""
...
def test_polygon_zkevm_normal_workflow():
    harness_dir = Path(__file__).resolve().parent
    contract_test = "PolygonZkEVMSourcedBridgeHarnessTest"
    ret, stdout, stderr = run_forge_test(harness_dir, contract_test)
    assert ret == 0, f"Foundry fixture harness failed for polygon_zkevm:\n{stderr}\n{stdout}"
...
if __name__ == "__main__":
    test_polygon_zkevm_normal_workflow()
