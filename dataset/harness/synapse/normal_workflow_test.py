"""Harness test executing a pinned-container EVM workflow for synapse (Synapse Protocol)."""
...
def test_synapse_normal_workflow():
    harness_dir = Path(__file__).resolve().parent
    contract_test = "SynapseSourceBackedHarnessTest"
    ret, stdout, stderr = run_forge_test(harness_dir, contract_test)
    assert ret == 0, f"Foundry fixture harness failed for synapse:\n{stderr}\n{stdout}"
