"""Harness test executing a pinned-container EVM workflow for celer_cbridge (Celer cBridge)."""
...
def test_celer_cbridge_normal_workflow():
    harness_dir = Path(__file__).resolve().parent
    contract_test = "CBridgePairedHarnessTest"
    ret, stdout, stderr = run_forge_test(harness_dir, contract_test)
    assert ret == 0, f"Foundry fixture harness failed for celer_cbridge:\n{stderr}\n{stdout}"
...
if __name__ == "__main__":
    test_celer_cbridge_normal_workflow()
