"""Harness test executing a pinned-container EVM workflow for arbitrum_token_bridge (Arbitrum Token Bridge)."""
...
def test_arbitrum_token_bridge_normal_workflow():
    harness_dir = Path(__file__).resolve().parent
    contract_test = "ArbitrumSourceBackedHarnessTest"
    ret, stdout, stderr = run_forge_test(harness_dir, contract_test)
    assert ret == 0, f"Foundry fixture harness failed for arbitrum_token_bridge:\n{stderr}\n{stdout}"
...
if __name__ == "__main__":
    test_arbitrum_token_bridge_normal_workflow()
