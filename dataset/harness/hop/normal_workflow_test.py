"""Harness test executing a pinned-container EVM workflow for hop (Hop Protocol)."""
...
def test_hop_normal_workflow():
    harness_dir = Path(__file__).resolve().parent
    contract_test = "HopPairedHarnessTest"
    ret, stdout, stderr = run_forge_test(harness_dir, contract_test)
    assert ret == 0, f"Foundry fixture harness failed for hop:\n{stderr}\n{stdout}"
