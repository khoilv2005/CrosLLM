"""Harness test executing a pinned-container EVM workflow for optimism (Optimism Bedrock)."""
...
def test_optimism_normal_workflow():
    harness_dir = Path(__file__).resolve().parent
    contract_test = "OptimismSourceBackedHarnessTest"
    ret, stdout, stderr = run_forge_test(harness_dir, contract_test)
    assert ret == 0, f"Foundry fixture harness failed for optimism:\n{stderr}\n{stdout}"
