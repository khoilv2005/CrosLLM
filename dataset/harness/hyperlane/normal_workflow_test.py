"""Harness test executing a pinned-container EVM workflow for hyperlane (Hyperlane)."""
...
def test_hyperlane_normal_workflow():
    harness_dir = Path(__file__).resolve().parent
    contract_test = "HyperlanePairedHarnessTest"
    ret, stdout, stderr = run_forge_test(harness_dir, contract_test)
    assert ret == 0, f"Foundry fixture harness failed for hyperlane:\n{stderr}\n{stdout}"
...
if __name__ == "__main__":
    test_hyperlane_normal_workflow()
