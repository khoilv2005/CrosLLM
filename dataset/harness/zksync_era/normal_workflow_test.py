"""Harness test executing a pinned-container EVM workflow for zksync_era (zkSync Era)."""
...
def test_zksync_era_normal_workflow():
    harness_dir = Path(__file__).resolve().parent
    contract_test = "ZkSyncEraSourceBackedHarnessTest"
    ret, stdout, stderr = run_forge_test(harness_dir, contract_test)
    assert ret == 0, f"Foundry fixture harness failed for zksync_era:\n{stderr}\n{stdout}"
    ...
    trace_record = {
        ...
        "status": "PASS",
        "evidence_scope": "source_backed_development_probe_deposit_escrow_and_finalize_forwarding",
    }
...
if __name__ == "__main__":
    test_zksync_era_normal_workflow()
