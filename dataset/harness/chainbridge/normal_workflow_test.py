"""Harness test executing a pinned-container EVM workflow for chainbridge (ChainBridge)."""
...
def test_chainbridge_normal_workflow():
    harness_dir = Path(__file__).resolve().parent
    contract_test = "ChainBridgePairedHarnessTest"
    ret, stdout, stderr = run_forge_test(harness_dir, contract_test)
    assert ret == 0, f"Foundry fixture harness failed for chainbridge:\n{stderr}\n{stdout}"
...
if __name__ == "__main__":
    test_chainbridge_normal_workflow()
