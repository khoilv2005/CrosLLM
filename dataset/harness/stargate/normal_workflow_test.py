"""Harness test executing a pinned-container EVM workflow for stargate (Stargate Finance)."""
...
def test_stargate_normal_workflow():
    harness_dir = Path(__file__).resolve().parent
    contract_test = "StargateSourceBackedHarnessTest"
    ret, stdout, stderr = run_forge_test(harness_dir, contract_test)
    assert ret == 0, f"Foundry fixture harness failed for stargate:\n{stderr}\n{stdout}"
