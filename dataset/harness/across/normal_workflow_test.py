"""Harness test executing a pinned-container EVM workflow for across (Across Protocol)."""
...
def test_across_normal_workflow():
    harness_dir = Path(__file__).resolve().parent
    contract_test = "AcrossSourceBackedHarnessTest"
    ret, stdout, stderr = run_forge_test(harness_dir, contract_test)
    assert ret == 0, f"Foundry fixture harness failed for across:\n{stderr}\n{stdout}"
