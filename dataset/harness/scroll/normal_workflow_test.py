"""Harness test executing a pinned-container EVM workflow for scroll (Scroll)."""
...
def test_scroll_normal_workflow():
    harness_dir = Path(__file__).resolve().parent
    contract_test = "ScrollSourceBackedHarnessTest"
    ret, stdout, stderr = run_forge_test(harness_dir, contract_test)
    assert ret == 0, f"Foundry fixture harness failed for scroll:\n{stderr}\n{stdout}"
    ...
    trace_record = {
        ...
        "status": "PASS",
        "evidence_scope": "source_backed_development_probe_enqueue_alias_relay_and_replay",
    }
